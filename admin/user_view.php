	...
	<div class="medium-9 columns">
    	...
    	<nav aria-label="You are here:" role="navigation">
        	<ul class="breadcrumbs">
          		...
          		<li><span class="show-for-sr">Current: </span> User</li>
        	</ul>
      	</nav>
      	<hr>
	    ...
	    <div class="button-group">
	    	<!--<a href="#" class="primary button"><i class="fa fa-plus"></i> Add</a>-->
        ...
        <tbody>
          <?php
          $i=0;
           $sql = 'SELECT * FROM `adidas_member` ORDER BY `adidas_member_id` DESC';
           foreach ($pdo->query($sql) as $row) {
           			$i++;
           			
                    echo '<tr>';
                    echo '<td>'. $i. '</td>';
                    echo '<td>'.$row['adidas_member_user'].'</td>';
                    echo '<td>'. $row['adidas_member_email'] . '</td>';
                    echo '<td>'. $row['adidas_member_status'] . '</td>';
                    echo '<td>'.$row['adidas_member_datetime'].'</td>';
                    if($row['check_active'] == 1){
                    	echo '<td>ยืนยันแล้ว</td>';
                    }else{
                    	echo '<td>ยังไม่ได้ยืนยัน</td>';
                    }
                    echo '</tr>';
           }
		  ?>
